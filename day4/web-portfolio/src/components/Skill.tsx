    ...
    <section className="mx-auto max-w-6xl px-6 mt-10 font-display" id="skills">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 my-8  gap-4 leading-normal text-[16px]">
        {Categories.map((cat) => (
          <div
            key={cat.id}
            className="px-10 py-6 rounded-2xl ring ring-zinc-800 bg-zinc-900/50 transition-all duration-300 hover:-translate-y-1 hover:ring-blue-500/40 hover:bg-zinc-900"
          >
            ...
            <div className="flex flex-row flex-wrap gap-2 mt-8 text-[14px] font-semibold">
              {cat.skill.map((skills) => (
                <span
                  key={skills}
                  className="border-2 border-zinc-800 bg-zinc-900/50 px-4 py-2 rounded-xl text-[#a1a1aa] transition-colors duration-200 hover:border-blue-500/40 hover:text-white"
                >
                  {skills}
                </span>
              ))}
